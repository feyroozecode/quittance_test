...
from .utils import json_default, get_menu_items, create_empty_report_template, allReportTypes
...
def edit(request):
    ...
    *create_demande_report_template* for this Demo App. Normally you'd probably
    ...
    if type_report not in allReportTypes:
        return HttpResponseBadRequest('invalid type_report parameter')
            
    if ReportDefinition.objects.filter(report_type=type_report).count() == 0:
        create_empty_report_template(type_report)

    # load ReportBro report definition stored in our report_definition table
    row = ReportDefinition.objects.get(report_type=type_report)
    ...
    if request.method == 'PUT':
        ...
        try:
            ...
            if output_format == 'pdf':
                ...
                print(report_request.report_definition)
                response = HttpResponse(
...
def save(request, report_type):
    ...
    """
    if report_type not in allReportTypes:
